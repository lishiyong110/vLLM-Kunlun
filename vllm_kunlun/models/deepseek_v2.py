...
def sparse_attn_indexer_vllm_kunlun(
    ...
    if has_decode:
        ...
        if decode_metadata.requires_padding:
            ...
            topk_indices_buffer[
                :num_decode_tokens, : topk_indices.shape[-1]
            ] = topk_indices
...
class Indexer(nn.Module):
    def __init__(
        ...
        ):  # 由于I8_paged_mqa_logits输入参数的限制，最大长度必须为block_size的整数倍
            self.max_model_len = (
                self.max_model_len
...
class DeepseekV2DecoderLayer(nn.Module):
    def __init__(
        ...
        # https://github.com/huggingface/transformers/pull/39847
        if hasattr(config, "rope_parameters"):
            ...
            if rope_type != "default":
                raise NotImplementedError(
                    f"Unsupported rope_type='{rope_type}' in rope_parameters. "
                    f"Only rope_type='default' is currently supported."
                )
